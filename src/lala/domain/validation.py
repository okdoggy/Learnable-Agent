...
from lala.renderers.remaster import REMASTER_ENGINE_VERSION
    ...
    def validate(
        ...
        for step in plan.steps:
            if isinstance(step, LutStep):
                self.catalog.resolve(step.parameters.lut_id)
        if capabilities:
            ...
            if capabilities.remaster_engine_version != REMASTER_ENGINE_VERSION.rsplit(".", 1)[0]:
                raise LalaError(
                    "REMASTER_VERSION_MISMATCH",
                    "Remaster 엔진 버전이 서로 다릅니다.",
                    False,
                )
            if (
