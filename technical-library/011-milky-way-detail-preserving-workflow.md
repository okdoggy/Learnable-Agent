...
summary_ko: 하늘·지상 분리 촬영, 선행 노이즈 제거, 별 보존형 현상, 광공해와 대기광 분리, 경계 합성을 연결한 은하수 처리 흐름이다.
version: 0.1.1
status: candidate
supported_tools:
- remaster
...
- raw-20260809-airglowmask
- raw-20260809-denoisefirst
- raw-20260809-skyground01
source_urls:
...
하늘·지상 분리 촬영, 선행 노이즈 제거, 별 보존형 현상, 광공해와 대기광 분리, 경계 합성을 연결한 은하수 처리 흐름이다.
...
1. 필요하면 별 디테일을 우선한 짧은 고감도 하늘 노출과 더 긴 지상 노출을 별도로 확보한다.
...
- 원문 저자의 일반적인 은하수 감도 상한 ISO 6400과 시험 파일 ISO 12800을 보편 최적값으로 일반화하지 않는다.
- Lens Sharpness Standard와 100% 검사는 해당 작업 흐름의 출발점이다.
- 셔터 시간·조리개·최종 Remaster 수치는 원문에 없으므로 만들지 않는다.

## 판단 근거

- 촬영, 노이즈 제거, 국소 광공해 보정, 합성을 잇는 완전한 흐름으로 결과 영향과 재사용 가치가 크다.
- Remaster의 denoise와 Generate AI compositing으로 일부 실행할 수 있다.
- 새 raw까지 포함한 핵심 근거가 동일 PetaPixel 원문 계열이어서 독립 근거 gate를 넘지 못하므로 candidate로 유지한다.

## 주의사항

- ISO 12800은 스트레스 테스트 사례이며 일반 권장 상한으로 오해하지 않는다.
- 강한 노이즈 제거와 Dehaze는 작은 별을 지우거나 인공적인 별 테두리를 만들 수 있다.
- 광공해와 자연 대기광을 같은 색 번짐으로 취급하지 않는다.
- 하늘·지상 합성 경계의 halo와 지상의 과도한 밝기를 확인한다.

## 충돌 및 예외

- 장면이 한 노출 범위에 들어오면 두 노출 합성을 무조건 사용하지 않는다.
